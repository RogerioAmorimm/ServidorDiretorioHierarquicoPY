...
class ServerDirRaiz(rpyc.Service):
    ...
    def exposed_lookup(self, serverName):
        ...
        if len(ListNames) > 0:
            ...
            if ListNames[NAMEROOT] in self.ListDir:
                print(f"Verificando se existe segunda camada")

                if len(ListNames) > 1:
                    secondLayerIP = self.ListDir[ListNames[NAMEROOT]][0]
                    secondLayerPort = self.ListDir[ListNames[NAMEROOT]][1]
...
if __name__ == "__main__":
    ...
    print(f"Iniciando servidor de diretórios na porta: {PORTDIRRAIZ}")
    serverDir = ThreadedServer(ServerDirRaiz, port=PORTDIRRAIZ)
    serverDir.start()
